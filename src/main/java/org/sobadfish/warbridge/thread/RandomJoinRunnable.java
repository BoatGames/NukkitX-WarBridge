...
import org.sobadfish.warbridge.manager.PlayerHasChoseRoomManager;
...
public class RandomJoinRunnable extends ThreadManager.AbstractBedWarRunnable {
    ...
    public boolean joinRandomRoom(RandomJoinManager.IPlayerInfo i){
        ...
        if (i.name == null) {
            ...
            WorldRoom worldRoom = WarBridgeMain.getMenuRoomManager().getRoom(i.name);
            ...
            if (roomConfigs.size() > 0) {
                ...
                if (WarBridgeMain.getRoomManager().hasGameRoom(roomConfig.name)) {
                    GameRoom fg = WarBridgeMain.getRoomManager().getRoom(roomConfig.name);
            ...
            if(WarBridgeMain.getMenuRoomManager().worldRoomLinkedHashMap.containsKey(i.name)){
                WorldRoom worldRoom = WarBridgeMain.getMenuRoomManager().getRoom(i.name);
    ...
    private boolean startGameRoom(PlayerInfo info, PlayerHasChoseRoomManager roomManager, GameRoomConfig roomConfig) {
        ...
        if(WarBridgeMain.getRoomManager().enableRoom(WarBridgeMain.getRoomManager().getRoomConfig(roomConfig.name))){
            if (WarBridgeMain.getRoomManager().getRoom(roomConfig.name).joinPlayerInfo(info, true) == GameRoom.JoinType.CAN_JOIN) {
                lock.remove(roomManager);
                return true;
            } else {
                lock.remove(roomManager);
